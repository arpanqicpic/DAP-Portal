...
const { getImageUrl } = require('../utils/AWS_file_converter');
...
router.get('/profile', async (req, res) => {
    ...
    try {
        ...
        const imageUrls = {};
        // Access image URLs and handle any missing or invalid fields
        for (const field of imageFields) {
          // Get the correct field based on its name (either from dist_documentDetails or distributer_invoice)
          const fieldName = field.name;
          let fileName;
          // Fetch the corresponding value (image file name) based on the field
          if (['invoice_id', 'agreement_id', 'welcome_poster_image_id', 'agreement_copy_id'].includes(fieldName)) {
            // Use the values from distributer_invoice
            fileName = distributer_invoice[fieldName];
          } else {
            // Use the values from dist_documentDetails for image-related fields
            fileName = dist_documentDetails[fieldName];
          }
          // Only fetch the image URL if the fileName exists
          if (fileName) {
            try {
              ...
              imageUrls[field.label] = null; // Optionally handle the error by setting it to null
            }
          } else {
            imageUrls[field.label] = null; // Handle cases where the fileName is not present
          }
        }
        ...
        const invoice = imageUrls.invoice_id;  // Accessing the invoice ID
        const agreement = imageUrls.agreement_id;  // Accessing the agreement ID
        const welcome_poster_image = imageUrls.welcome_poster_image_id;  // Accessing the welcome poster image ID
        const agreement_copy = imageUrls.agreement_copy_id;  // Accessing the agreement copy ID
